...
class TestAccountable(object):
    ...
    def test_projects(self, mock_object, tmpdir):
        ...
        mock_object.return_value = support.metadata_response().json()
    ...
    def test_checkoutbranch(self, mock_get, mock_post, tmpdir):
        mock_post.return_value = support.issue_create().json()
        mock_get.return_value = support.metadata_response().json()
        a = support.accountable(tmpdir)
